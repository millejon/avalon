...
def test_update_database(app, album_directory, artist_count, hub_count):
    with app.app_context():
        files = util.get_song_file_paths(album_directory)
        ...
        assert len(db.execute_read_query("""SELECT * FROM albums""")) == len(metadata)
        assert len(db.execute_read_query("""SELECT * FROM songs""")) == len(metadata)
